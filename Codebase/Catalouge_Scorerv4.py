...
os.environ["HUGGINGFACEHUB_API_TOKEN"] = "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"

llm = HuggingFaceHub(
    repo_id="mistralai/Mistral-7B-Instruct-v0.2", model_kwargs={"temperature": 0.5, "max_new_tokens": 10000}
)
...
def model(question):
    ...
    response_index = response.find("Question: {question}")
    if response_index != -1:
        response=response[response_index+20:]
